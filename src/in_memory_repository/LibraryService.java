...
import in_memory_repository.interfaces.IExceptionSupplier;
...
public class LibraryService implements ILibraryService {
    ...
    public void saveBook(Book book) {
        var r1 = ValidationUtils.validate(book, titleNotEmpty);
        if (r1.isError()) {
            IPrinter.err("Fehler Speicherung Buch: " + r1.getError());
            return;
        }
        var r2 = ValidationUtils.validate(book, authorNotEmpty);
        if (r2.isError()) {
            IPrinter.err("Fehler Speicherung Buch: " + r2.getError());
            return;
        }
        var r3 = ValidationUtils.validate(book, isbnFormatValid);
        if (r3.isError()) {
    ...
    public void saveMember(Member member) {
        var r1 = ValidationUtils.validate(member, memberNameNotEmpty);
        if (r1.isError()) {
            IPrinter.err("Fehler Speicherung Member: " + r1.getError());
            return;
        }
        var r2 = ValidationUtils.validate(member, memberEmailValid);
        if (r2.isError()) {
